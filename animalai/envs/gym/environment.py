...
class AnimalAIEnv(gym.Env):
    ...
    def reset(self, arenas_configurations=None):
        ...
        Returns: observation (object/list): the initial observation of the
            space.
        """
        minfo = self._env.reset(arenas_configurations=arenas_configurations)[self.brain_name]
        n_agents = len(minfo.agents)
        self._check_agents(n_agents)
        self.game_over = False

        if self._n_agents == 1:
            obs, reward, done, info = self._single_step(minfo)
        else:
            obs, reward, done, info = self._multi_step(minfo)
        return obs
